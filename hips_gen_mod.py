        ...
        for tile in tiles:
            ...
            if mode == 'image':
                try:
                    ...
                    print(f"an unexpected error occurred: {e} for file {fn[0]}")
...
def main():
    ...
    for insuffix, mode in (
        ...
        infnameglob = f'e?01_%06i_{insuffix}_{emin}_{emax}_c{procver}.fits.gz'
        outrootdir = f'{outroot}/{outprefix}_{insuffix}_{emin}_{emax}_c{procver}'
    ...
    # compute rates
    outrootdir = f'{outroot}/{outprefix}_Rate_{emin}_{emax}_c{procver}'
    if not os.path.exists(outrootdir):
        os.makedirs(outrootdir)
    ...
    for fname_cts in glob.iglob(os.path.join(outdirs[0], f'Norder{maxorder}', 'Dir*', 'Npix*.fits')):
        ...
        exp_dir = f'{outroot}/{outprefix}_ExposureMap_{emin}_{emax}_c{procver}'
