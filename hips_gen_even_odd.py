...
inrootdir='/Users/ethlau/Research/eRASS1/data'
...
def extractPixels(outrootdir, infnameglob, norder, imgorder, mode='image', sample=None):
    ...
    def process_hp_map(base):
        ...
        for tile in tiles:
            ...
            if mode == 'image':
                ...
                #if sample == 'odd' or sample == 'even':
                    #polysums = polysums/2.0
    ...
    with forkqueue.ForkQueue(ordered=False, numforks=6, env=locals()) as q:
        for out in q.process(process_hp_map, ((i,) for i in range(nmaps) )):
            pass
...
def main():
    ...
    outroot = '/Users/ethlau/Research/eRASS1'
    procver = '010'
    outprefix = 'eRASS1'
    #for band in range(0, 7+1):

    for insuffix, mode in (
            ('Image', 'events'),
            ('ExposureMap', 'image'),
        ):

        infnameglob = f'e?01_%06i_{insuffix}_c{procver}.fits.gz'
        outrootdir = f'{outroot}/{outprefix}_{insuffix}_c{procver}'

        if not os.path.exists(outrootdir):
            os.makedirs(outrootdir)
        
        extractPixels(outrootdir, infnameglob, maxorder, imgorder, mode=mode, sample=None)


    # counts and exposure    
    for sample in ['odd', 'even'] :
        outdirs = []
        ...
        for insuffix, mode in (
            ...
            infnameglob = f'e?01_%06i_{insuffix}_c{procver}.fits.gz'
            outrootdir = f'{outroot}/{outprefix}_{insuffix}_c{procver}_'+sample
            outdirs.append(outrootdir)

            if not os.path.exists(outrootdir):
                os.makedirs(outrootdir)
        
            extractPixels(outrootdir, infnameglob, maxorder, imgorder, mode=mode, sample=sample)
        ...
        # compute rates
        outrootdir = f'{outroot}/{outprefix}_Rate_c{procver}_'+sample
        if not os.path.exists(outrootdir):
            os.makedirs(outrootdir)
        ...
        for fname_cts in glob.iglob(os.path.join(outdirs[0], f'Norder{maxorder}', 'Dir*', 'Npix*.fits')):
            ...
            exp_dir = f'{outroot}/{outprefix}_ExposureMap_c{procver}'
            fname_exp = os.path.join(exp_dir, '/'.join(p[-3:]))
